...
router.post("/add", async (req, res, next) => {
  try {
    ...
    res
      .status(201)
      .json({ status: 200, message: "Category added successfully", result });
  } catch (err) {
    next(err);
  }
});
router.get("/view", async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "All Categories", allCategories });
  } catch (err) {
    next(err);
  }
});
...
router.delete("/delete/:id", async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "Category deleted successfully", result });
  } catch (err) {
    next(err);
  }
});
